...
if (file.exists("./project/volume/data/processed/teamstats(roll7).csv")) {
  file.remove("./project/volume/data/processed/teamstats(roll7).csv")}
...
for (i in 1:length(NFLvalues)) {
  teamAllowed[, new_column := Reduce(`+`, shift(get(NFLvalues[i]), 1:7))]
  setnames(teamAllowed,"new_column",paste0("roll_7_",NFLvalues[i]))
}




fwrite(teamAllowed, "./project/volume/data/processed/teamstats(roll7).csv")